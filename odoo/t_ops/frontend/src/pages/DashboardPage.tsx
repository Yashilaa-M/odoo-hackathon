...
import { AlertCircle, BrainCircuit, Gauge, HelpCircle, Route, ShieldAlert, Target, Truck, UserCheck, Wrench } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1.25fr_0.75fr] gap-6">
        <LiveFleetActivity compact />
        <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-1">
          <div className="relative overflow-hidden rounded-3xl border border-border bg-transit-surface2/86 p-6 shadow-glass">
            <div className="absolute inset-0 bg-[linear-gradient(120deg,rgba(34,211,238,0.14),transparent_42%,rgba(196,113,237,0.12))]" />
            <div className="relative">
              <div className="mb-5 flex items-center justify-between">
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-transit-text-secondary">Network Pulse</p>
                  <h3 className="mt-2 text-2xl font-black text-foreground">{kpis?.fleetUtilization || 0}%</h3>
                </div>
                <Gauge className="h-8 w-8 text-cyan-300" />
              </div>
              <div className="h-3 overflow-hidden rounded-full bg-transit-surface3">
                <div
                  className="h-full rounded-full bg-gradient-primary shadow-[0_0_22px_rgba(34,211,238,0.35)]"
                  style={{ width: `${Math.min(kpis?.fleetUtilization || 0, 100)}%` }}
                />
              </div>
              <p className="mt-4 text-sm text-transit-text-secondary">Fleet load is tracked against dispatch, shop, and availability status in real time.</p>
            </div>
          </div>

          <div className="rounded-3xl border border-border bg-transit-surface2/86 p-6 shadow-glass">
            <div className="mb-5 flex items-center justify-between">
              <div>
                <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-transit-text-secondary">Risk Matrix</p>
                <h3 className="mt-2 text-lg font-black text-foreground">Compliance guardrails</h3>
              </div>
              <ShieldAlert className="h-8 w-8 text-amber-300" />
            </div>
            <div className="grid grid-cols-3 gap-2 text-center">
              <div className="rounded-2xl bg-emerald-400/10 p-3 text-emerald-300">
                <div className="text-xl font-black">{kpis?.driversOnDuty || 0}</div>
                <div className="text-[10px] uppercase tracking-[0.16em]">Ready</div>
              </div>
              <div className="rounded-2xl bg-amber-400/10 p-3 text-amber-300">
                <div className="text-xl font-black">{kpis?.inShopVehicles || 0}</div>
                <div className="text-[10px] uppercase tracking-[0.16em]">Shop</div>
              </div>
              <div className="rounded-2xl bg-cyan-400/10 p-3 text-cyan-300">
                <div className="text-xl font-black">{kpis?.activeTrips || 0}</div>
                <div className="text-[10px] uppercase tracking-[0.16em]">Trips</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Smart Insights Card */}
        <div className="bg-card border border-border rounded-3xl p-6 shadow-glass lg:col-span-2 space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {insights.map((ins, idx) => (
              <div key={idx} className="rounded-2xl border border-border bg-transit-surface3/55 p-4">
                <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-2xl bg-cyan-300/10 text-cyan-300">
                  {idx === 0 ? <Target className="h-5 w-5" /> : idx === 1 ? <AlertCircle className="h-5 w-5" /> : <BrainCircuit className="h-5 w-5" />}
                </div>
                <p className="text-xs leading-5 text-foreground">{ins}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-card border border-border rounded-3xl p-6 shadow-glass space-y-4">
          <div className="flex items-center gap-2 border-b border-border pb-3">
